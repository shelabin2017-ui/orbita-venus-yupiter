...
from .db import Block, Match, Photo, Reaction, User
...
async def is_vip(u):
    return bool(u.vip_until and u.vip_until > datetime.utcnow())
...
async def like_count_today(s, u):
    ...
    return await s.scalar(
        ...
        .where(
            Reaction.from_user_id == u.id,
            Reaction.kind.in_(["like", "spark"]),
            Reaction.created_at >= start,
        )
    )
...
async def next_profile(s, me, radius=None, inactive_days=7):
    ...
    q = select(User).where(
        ...
        or_(User.last_active_at.is_(None), User.last_active_at >= freshness),
        ~User.id.in_(seen),
    )
    if me.looking_for and me.looking_for != "any":
        q = q.where(User.gender == me.looking_for)

    rows = (
        await s.execute(
            q.order_by(User.last_active_at.desc().nullslast(), User.id).limit(100)
        )
    ).scalars().all()

    for user in rows:
        if await blocked(s, me.id, user.id):
            continue
        if radius:
...
async def react(s, me, other, kind):
    old = (
        await s.execute(
            select(Reaction).where(
                Reaction.from_user_id == me.id,
                Reaction.to_user_id == other.id,
            )
        )
    ).scalar_one_or_none()
    ...
    if old:
        ...
        s.add(Reaction(from_user_id=me.id, to_user_id=other.id, kind=kind))
    await s.commit()

    if kind not in {"like", "spark"}:
        return False

    reciprocal = (
        await s.execute(
            select(Reaction).where(
                Reaction.from_user_id == other.id,
                Reaction.to_user_id == me.id,
                Reaction.kind.in_(["like", "spark"]),
            )
        )
    ).scalar_one_or_none()
    ...
    if not exists:
        s.add(Match(user_a_id=a, user_b_id=b))
        await s.commit()
    return True
