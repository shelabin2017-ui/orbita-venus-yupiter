...
async def main():
    cfg=load_config(); db=DB(cfg.database_url); redis=make_redis(cfg.redis_url); bot=Bot(cfg.bot_token,default=DefaultBotProperties(parse_mode=ParseMode.HTML)); dp=Dispatcher(storage=make_fsm_storage(redis))
    async def inject(handler,event,data):
        user=getattr(event,"from_user",None); set_current_user_is_admin(bool(user and user.id in cfg.admin_ids)); data.update(db=db,config=cfg,redis=redis)
    ...
    try:
        ...
        moderation_task.cancel(); inactivity_task.cancel(); await asyncio.gather(moderation_task,inactivity_task,return_exceptions=True); await bot.session.close(); await redis.aclose(); await db.engine.dispose()
